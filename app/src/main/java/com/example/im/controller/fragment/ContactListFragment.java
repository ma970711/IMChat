...
public class ContactListFragment extends EaseContactListFragment {
    ...
    private BroadcastReceiver NewGroupReceiver = new BroadcastReceiver() {
        @Override
    ...
    protected void initView() {
        super.initView();
        titleBar.setRightImageResource(R.drawable.em_add);//添加右上角加号
    ...
    protected void setUpView() {
        ...
        mLBM.registerReceiver(NewGroupReceiver,new IntentFilter(Constant.NEW_GROUP_INVITE));

        mLBM.registerReceiver(GroupChangeReceiver,new IntentFilter(Constant.GROUP_INVITE_CHANGED));
        //从环信服务器中获取所有联系人信息
        getContanctFromHxServer();
    ...
    private void getContanctFromHxServer() {
        Model.getInstance().getGlobalThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    if (hxids != null && hxids.size()>=0){
                        List<UserInfo> contants = new ArrayList<UserInfo>();
                        //转换
                        for (String hxid : hxids){
                            UserInfo userInfo = new UserInfo(hxid);
                            contants.add(userInfo);
                        }
                        //保存好友信息到本地数据库
                        Model.getInstance().getDbManager().getContactTableDao().saveContacts(contants,true);
                        if (getActivity() == null){
                            return;
                        }
                        //刷新页面
    ...
    public void onDestroy() {
        ...
        mLBM.unregisterReceiver(NewGroupReceiver);
    }
}
